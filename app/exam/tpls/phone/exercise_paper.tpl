...
{x2;endif}
		...
		<div class="row-fluid" style="margin-bottom:2.5rem;">
			<div class="container-fluid">
				...
				<form id="form1" name="form1" method="post" action="index.php?exam-phone-exercise-score" class="col-xs-12" style="padding:0px;margin-bottom:4.8rem;" data-page="paper" data-target="paper">
					...
					<div class="paperexamcontent" id="questions_{x2;v:question['questionid']}" rel="{x2;v:quest}" style="clear:both;overflow:hidden;background:#FFFFFF;margin-top:0.5rem;padding:1rem;">
						...
						<div class="selector">
	                    	{x2;if:$questype[v:quest]['questchoice'] == 1 || $questype[v:quest]['questchoice'] == 4}
		                        ...
		                        {x2;eval: break;}
		                        {x2;endif}
		                        <label class="radio-inline" style="line-height:2.8rem;"><input type="radio" name="question[{x2;v:question['questionid']}]" rel="{x2;v:question['questionid']}" value="{x2;v:so}" {x2;if:v:so == $sessionvars['examsessionuseranswer'][v:question['questionid']]}checked{x2;endif}/>{x2;v:so} </label>
		                        {x2;endtree}
	                        ...
	                        {x2;else}
		                        ...
		                        {x2;eval: break;}
		                        {x2;endif}
		                        <label class="checkbox-inline" style="line-height:2.8rem;"><input type="checkbox" name="question[{x2;v:question['questionid']}][{x2;v:key}]" rel="{x2;v:question['questionid']}" value="{x2;v:so}" {x2;if:in_array(v:so,$sessionvars['examsessionuseranswer'][v:question['questionid']])}checked{x2;endif}/>{x2;v:so} </label>
		                        {x2;endtree}
	                        {x2;endif}
	                    </div>
						{x2;else}
					...
					<div class="paperexamcontent" id="questions_{x2;v:data['questionid']}" rel="{x2;v:quest}" style="clear:both;overflow:hidden;background:#FFFFFF;margin-top:0.5rem;padding:1rem;">
						<h4 class="title">第{x2;v:tid}题{x2;v:did}小题
							<span class="pull-right">
								...
								<a class="btn {x2;if:$sessionvars['examsessionsign'][v:data['questionid']]}text-danger{x2;else}text-info{x2;endif} qicon" href="javascript:;" onclick="javascript:signQuestion('{x2;v:data['questionid']}',this);"><i class="glyphicon glyphicon-bookmark"></i></a>
		                ...
		                <div style="padding:0rem 1.5rem;margin-top:1rem;">
							...
							<div class="selector">
		                    	{x2;if:$questype[v:quest]['questchoice'] == 1 || $questype[v:quest]['questchoice'] == 4}
			                        ...
			                        {x2;eval: break;}
			                        {x2;endif}
			                        <label class="radio-inline" style="line-height:2.8rem;"><input type="radio" name="question[{x2;v:data['questionid']}]" rel="{x2;v:data['questionid']}" value="{x2;v:so}" {x2;if:v:so == $sessionvars['examsessionuseranswer'][v:data['questionid']]}checked{x2;endif}/>{x2;v:so} </label>
			                        {x2;endtree}
			                    {x2;elseif:$questype[v:quest]['questchoice'] == 5}
		                        ...
		                        {x2;else}
			                        ...
			                        {x2;eval: break;}
			                        {x2;endif}
			                        <label class="checkbox-inline" style="line-height:2.8rem;"><input type="checkbox" name="question[{x2;v:data['questionid']}][{x2;v:key}]" rel="{x2;v:data['questionid']}" value="{x2;v:so}" {x2;if:in_array(v:so,$sessionvars['examsessionuseranswer'][v:data['questionid']])}checked{x2;endif}/>{x2;v:so} </label>
			                        {x2;endtree}
		                        {x2;endif}
		                    </div>
							{x2;else}
		...
		<div class="container-fluid text-center" style="background-color:#2DAB49;z-index:9999;position:fixed;bottom:0px;border-top:1px solid #E9E9E9;">
			<div style="clear:both;background-color:#2DAB49;padding-top:0px;border:0px;padding:0rem;padding-bottom:0.8rem;" class="input-group-addon footer">
				<div class="col-xs-3">
